...
export const Similar = memo(({ mainGame }: { mainGame: IGame }) => {
  const similarGames = useAppSelector((store) => store.games?.gamesList)?.filter(
    (game) =>
      game.name !== mainGame?.name &&
      game.genres.some(
        (gameGenre) =>
          gameGenre.description === mainGame?.genres[0]?.description ||
          gameGenre.description === mainGame?.genres[1]?.description ||
          gameGenre.description === mainGame?.genres[2]?.description,
      ),
  );
